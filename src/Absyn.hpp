...
public:
    ...
    virtual void visitLVal(LVal *p) = 0;
    ...
    virtual void visitLIdent(LIdent *p) = 0;

    virtual void visitLArray(LArray *p) = 0;
    ...
    virtual void visitEStruct(EStruct *p) = 0;
...
class LVal : public Visitable {
public:
    virtual LVal *clone() const = 0;
...
protected:
    virtual bool isEqual(const Type& t2);
public:
    virtual Type *clone() const = 0;
    virtual const TypeSpecifier getTypeSpecifier() const = 0;
    virtual std::string getTranslation() const = 0;
    virtual bool operator==(const Type& t2);
...
public:
    ...
    StructDef(Ident p1, ListAttribute *p2);
...
class LIdent : public LVal {
public:
    Ident ident_;

    LIdent(const LIdent&);

    LIdent& operator=(const LIdent&);
    ...
    virtual LIdent *clone() const;

    void swap(LIdent&);
};

class LArray : public LVal {
public:
    LVal *lval_;
    Expr *expr_;
...
private:
    bool isEqual(const Type& t2) override {
        auto t2_casted = dynamic_cast<const Array&>(t2);
        return *type_ == *t2_casted.getType_();
    }
public:
    Type *type_;
    size_t size = 0;
